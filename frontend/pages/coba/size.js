...
const Size = () => {
  ...
  const [sizeDataAdd, setSizeDataAdd] = useState({
    name: "",
    deskripsi: "",
    id_product: "",
  });

  const [sizeDataUpdate, setSizeDataUpdate] = useState({
    id: "",
    name: "",
    deskripsi: "",
    id_product: "",
  ...
  const sizeDetails = (id) => {
    router.push(`/dashboard/sub/size/${id}`);
  };
  useEffect(() => {
  ...
  const handleStatus = async (sizeId, currentStatus) => {
    try {
      setLoading(true);

      const newStatus = currentStatus === 0 ? 1 : 0;

      const response = await client.put(`/api/size/${sizeId}`, {
      ...
      setSizes((prevSizes) =>
        prevSizes.map((size) =>
          size._id === sizeId ? { ...size, status: newStatus } : size
        )
      );
    } finally {
      setLoading(false);
  ...
  useEffect(() => {
    const fetchSizes = async () => {
      try {
        ...
        // Set the fetched sizes into state
  ...
  const handleUpdateExtra = (size) => {
    setExtraToUpdate(size); // Menyimpan Varian yang dipilih
    setIsUpdateModalOpen(true);
  ...
  const deleteExtraById = async (id) => {
    ...
    if (result.isConfirmed) {
      try {
        const token = localStorage.getItem("token");
        const response = await client.delete(`/api/size/${id}`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
        ...
        if (response.status === 200) {
          ...
          setSizes((prevSizes) => prevSizes.filter((p) => p._id !== id));
        }
      } catch (error) {
  ...
  const handleSubmitUpdate = async (e) => {
    ...
    try {
      // const sizeId = "67a9615bf59ec80d10014871";
      const token = localStorage.getItem("token");
      const response = await client.put(
        `/api/size/${extraDataUpdate.id}`,
        {
                ...
                <tbody>
                  {sizes.map((size, index) => (
                    <tr key={size._id}>
                      ...
                      <td>{size.deskripsi}</td>
                      <td className="flex space-x-4">
                        {" "}
                        {/* Beri jarak antar tombol */}
                        <button
                          className=" p-3 rounded-lg text-2xl "
                          onClick={() => deleteExtraById(size._id)}
                        >
                          <MdDelete />
                        </button>
                        <button
                          className=" p-3 rounded-lg text-2xl "
                          onClick={() => handleUpdateExtra(size)}
                        >
                          <FaRegEdit />
                        </button>
                        <button
                          className=" p-3 rounded-lg text-2xl "
                          onClick={() => sizeDetails(size._id)}
                        >
                          <FaInfoCircle />
                        </button>
                      </td>
